...
(function () {
    ...
    this.historyView = (function () {
        ...
        var templates = {
            'row': $('#tmpl-entry-row').html(),
            'day-row': $('#tmpl-day-row').html()
        };
        ...
        $(historyModel).bind('modelrefresh', function () {
            ...
            $.each(results_day, function (day, items) {
                $(Mustache.render(templates['day-row'], {date: new Date(parseInt(day, 10)).toDateString()})).appendTo($table);
                // on each day populate the results that corresponds to that day.
                $.each(items, function (i, visit) {
                    visit['isSelected'] = function () {
                        return function (text, render) {
                            return historyModel.isSelected(this.id, this.day) ? render(text) : '';
                        }
                    }
                    var row = Mustache.render(templates['row'], visit);
                    $table.append(row);
                });
            });
        });
