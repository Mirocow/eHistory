...
(function () {
     ...
     *
     * title:title url:url site:site startTime:startime endTime:endtime searchquery
     */
    ...
    $(function () {
        ...
        var updateMainSearchBox = function (event) {
        ...
        $('input', $pnlAdvanced)
            .change(updateMainSearchBox)
            .keyup(updateMainSearchBox);

        $('.open-advanced').click(function (event) {
            $('.advanced-search').show();
            $('.open-advanced').hide();
            $('.advanced-search').find("input#pure-text").focus();
            return false;
        });

        $('.advanced-search input').focusout(function (event) {
            if($(event.relatedTarget).parents('.advanced-search').length == 0) {
                $('.advanced-search').hide();
                $('.open-advanced').show();
            }
            return false;
        ...
        function search(config) {
            ...
            var startTime = config.settings.startTime,
                endTime = config.settings.endTime,
                text = config.settings.text,
                filters = config.filters,
                settings = {
                    text: text || '',
                    startTime: new Date(startTime || 0).getTime(),
                    endTime: new Date(endTime || Date.now()).getTime(),
                    maxResults: historyModel.pageSize
                };

            historyView.displayThrobber();

            EHistory.search(settings, filters, function (results) {
